...
def load_data_pairs(pair_list, resize_r, rename_map):
    ...
    for k in range(0, len(pair_list), 2):
        ...
        resize_dim = np.array([img_data.shape[0], img_data.shape[1], img_data.shape[2]]).astype('int')
...
# process .nii data
def load_data_pairs_padding(pair_list, resize_r, rename_map, pad_w):
    """load all volume pairs and pad with zero"""
    img_clec = []
    label_clec = []
    ...
    for k in range(0, len(pair_list), 2):
        ...
        lab_data = resize(lab_data, resize_dim, order=0, preserve_range=True)
        # pad
        img_data_pad = np.lib.pad(img_data, ((pad_w, pad_w), (pad_w, pad_w), (pad_w, pad_w)), 'constant',
                              constant_values=np.array(((0, 0), (0, 0), (0, 0))))
        lab_data_pad = np.lib.pad(lab_data, ((pad_w, pad_w), (pad_w, pad_w), (pad_w, pad_w)), 'constant',
                              constant_values=np.array(((0, 0), (0, 0), (0, 0))))

        lab_rename = np.zeros(lab_data_pad.shape, dtype='int32')
        # rename labels
        for i in range(len(rename_map)):
            lab_rename[lab_data_pad == rename_map[i]] = i

        # for s in range(img_data_pad.shape[2]):
        #     cv2.imshow('img', np.concatenate(((img_data_pad[:,:,s]).astype('uint8'), (lab_rename[:,:,s]*30).astype('uint8')), axis=1))
        #     cv2.waitKey(50)

        img_clec.append(img_data_pad)
...
def remove_minor_cc(vol_data, rej_ratio, class_n, rename_map):
